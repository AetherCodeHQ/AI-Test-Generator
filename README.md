...
> AI-Test-Generator - Open source tool by AetherCodeHQ

`ai` `cli` `developer-tools` `golang` `open-source` `testing`

## What is it?

**Ai Test Generator** is a ai/ml tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/AI-Test-Generator.git
...
```bash
# Basic usage
./AI-Test-Generator --path ./target

# With options
./AI-Test-Generator --path ./target --format json --output report.json

# Verbose mode
./AI-Test-Generator --path ./target --verbose
```
...
### Basic scan
```bash
./AI-Test-Generator --path ./my-project
```

### JSON report
```bash
./AI-Test-Generator --path ./my-project --format json --output report.json
```
...
- name: Run Ai Test Generator
  run: ./AI-Test-Generator --path . --format json --output report.json
```
